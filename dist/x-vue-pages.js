...
var script = {
  ...
  methods: {
    onPick: function onPick(_ref, type) {
      ...
      this.minPickerDate[type] = dayjs_min(minDate).format('YYYY-MM-DD 00:00:00');
    },
    disabledDate: function disabledDate(time, maxLimit, type, minDate) {
      var currDateDiff = dayjs_min().diff(dayjs_min(time), 'day') > minDate;
      ...
      if (this.minPickerDate[type] && maxLimit) {
        var diff = dayjs_min(this.minPickerDate[type]).diff(dayjs_min(), 'day') < maxLimit - minDate;
        // 有最小日期 和 日期范围限制
        // 只有当 最小日期存在 且 选择的第一个时间 小于（日期范围限制 - 最小日期）
        if (minDate && diff) {
          var min = dayjs_min().subtract(minDate, 'day').format('YYYY-MM-DD 00:00:00');
          var max = dayjs_min(this.minPickerDate[type]).add(maxLimit, 'day').format('YYYY-MM-DD 23:59:59');
          var curr = dayjs_min(time).format('YYYY-MM-DD HH:mm:ss');
          return !isDuringDate(curr, min, max);
        }
        // 只有日期范围限制
        var flag = Math.abs(dayjs_min(time).diff(dayjs_min(this.minPickerDate[type]), 'day')) > maxLimit;
        return flag;
      }
      return false;
    },
...
var script$5 = {
  ...
  methods: {
    init: function init() {
      ...
      this.formList = lodash.cloneDeep(form).map(function (item) {
        return lodash.omit(item, ['defaultValue']);
      });
        ...
        return _regeneratorRuntime().wrap(function _callee$(_context) {
          while (1) switch (_context.prev = _context.next) {
            ...
            case 14:
              ...
              _this.cacheSearchForm = lodash.cloneDeep(searchParams);
              if (!mockData) {
                _context.next = 22;
                break;
              }
            ...
            case 22:
              ...
              request(requestConfig).then(function (_ref) {
                var code = _ref.code,
                  data = _ref.data;
                if (code === 200) {
                  var list = responseListPath ? lodash.get(data, responseListPath, []) : data;
                  _this.list = transformResponse ? transformResponse(list) : list;
                  _this.pagination.total = responseTotalPath ? lodash.get(data, responseTotalPath, 0) : data;
                }
              })["finally"](function () {
                _this.loading = false;
              });
